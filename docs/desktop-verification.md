...
| 6. (grok) first run | S5 | 2026-09-16 | grok | fail | composer=`textarea` (hidden) | — | user report: never submitted — text landed in the hidden helper textarea; fixed by the grok composer/send cascade change |
| 6. (grok) re-run after the cascade fix | S5 | 2026-09-16 | grok | pass | composer=`div.tiptap.ProseMirror[contenteditable='true'][aria-label='Ask Grok anything']`, send=`button[data-testid='chat-submit']` | — | user report after relaunch: "This looks like it worked!" — split-mode send now submits on all three sites |
...
| 19. `chats.json` holds the three chat URLs; select an older conversation → panes navigate to its chats; a Send continues them; New conversation → fresh chats. | S6 | 2026-09-18 | all | pass | — | — | chats.json holds all three links in the real format (chatgpt's is a real /c/<uuid>, not the placeholder), verified against a live Fusion conversation |
| 20. Unchecked target → that site untouched, `turn_start.slots` lists the subset, Analyze reports it missing. | S6 | 2026-09-18 | grok | pass | — | — | `slots:['grok']` sent to grok alone; the other two panes were never touched |
| 21. Break a selector in `selectors.json` → chip red within 2 s without restart; restore. | S6 | | | | | | |
| 22. Start a unified Send while a site is still replying to a manual prompt → `view_busy` for that slot, others proceed. | S6 | 2026-09-18 | grok | pass | — | view_busy | a second CONVERSATION sending to a site that was mid-reply was refused with view_busy before anything was typed, while the first completed (took two attempts: the first delay was longer than grok's 2.9 s reply, so there was no overlap to collide with) |
| 23. Analyst = ChatGPT web session (default) → Analyze produces a report; reveal the analyst tab and confirm a fresh chat per Analyze and only R1/R2/R3 in what was typed. | S7 | 2026-09-18 | chatgpt | pass | — | — | Analyze ran on the hidden ChatGPT analyst page; the anonymisation firewall was scanned on LIVE traffic: each model's typed challenge carried only the OTHER two R-labels (claude saw R2/R3, chatgpt R1/R3, grok R1/R2) and no forbidden identity string, product name, slot id or codename |
| 23. (analyst capture at raised effort) | S10 | 2026-09-20 | chatgpt | fail→fixed | assistant `[data-message-author-role='assistant']`, text `.markdown` | timeout | With a reasoning mode switched on inside ChatGPT, Analyze degraded every run: the capture ended mid-typing and returned 13 then 6 characters stamped `ok` (```JSON\n{\n``` and {"agre). Fixed by the S10 JSON gate plus four capture fixes. The live re-run then failed DIFFERENTLY — `timeout ms=302755 done_by=- chars=0`, nothing captured in the 300 s a pane is given — while a read-only probe of that same chat showed ChatGPT had answered correctly (5,847 chars of a valid claims object) and the real adapter read it back at 5,854 chars with looksComplete true. So: selectors right, prompt right, budget too short for a thinking model. The analyst now scales every capture window (not only the stillness ones), capped under the deadline the backend granted, and a container that never holds text reports `reply_not_found` naming that rather than a generic timeout. Re-run still to do. |
| 24. Fusion with 1 iteration → challenges typed visibly into the three panes, JSON replies captured, stances shown; the retry path observed at least once. | S7 | 2026-09-18 | all | pass | — | — | a real Fusion(1) round: 3 defends, nobody revised, exit `stalemate` with d1 standing; every site's thread holds the [chat, chat, fusion_challenge, fusion_reply] sequence, so the challenge really landed in that site's own conversation |
| 25. Analyst = `ollama:hermes3` → Analyze produces a report or degrades cleanly with raw attempts shown. | S7 | 2026-09-18 | — | pass | — | — | analyst = `ollama:hermes3`: one retry then a valid extraction of the same divergence, 1498/473 tokens, cost 0, entirely local |
| 26. Analyst unset → Analyze disabled with the hint; a pre-pivot conversation (OpenRouter models) → Send yields `transport_disabled`, nothing reaches OpenRouter. | S7 | 2026-09-18 | all | pass | — | transport_disabled | a conversation whose slots and analyst were pre-pivot OpenRouter slugs was refused on all three slots before any site call; separately, the launcher was shown to pin OPENROUTER_API_KEY to empty and the transport to refuse OpenRouter even with a real key in the environment |
...
| 28. (claude) capture fidelity | S7 | 2026-09-18 | claude | fail→fixed | `assistantText` was empty → the whole turn was captured | — | a real Send captured claude's thinking-summary line TWICE before the answer (the answer itself was intact; chatgpt and grok were clean). Cause: claude renders that summary in both the visible row and a height-collapsed panel. Fixed by `assistantText: ['.prose']` plus a shadow-root-aware nested-match filter; fake-site `?thinking=1` reproduces it as a regression |
| 29. Pre-parse (2026-09-23): type a question → Pre-parse → within ~2 min the composer holds the restated question with the answer block appended, Undo restores the typed text; Cancel mid-wait unlocks the composer AND the analyst page stops (reveal the analyst tab: no reply keeps streaming — the offline cancel test cannot see the live site); Send → the three replies come back under 4,000 characters in the `Key claims:` shape; Refactor = 4 analyst calls, Analyze = 1 (`bridge req=` lines, `raw_attempts` counts). | S12 | | | | | | |
...
left behind (`PDF_CHECK_RUNS=6` for a longer run). It is a canary for a Chromium teardown behaviour
that neither `node --test` nor the app spec can see: on Electron 44.4.1 / Chromium 152 (measured
2026-09-18), tearing the offscreen print window down with `destroy()` rather than `close()` made the
SECOND render in the process fail with `ERR_FAILED (-2)` on its own `file://` document and the THIRD
take the browser process down with SIGTRAP. That happens only while the print window is the LAST
window in the process, which is exactly the shape this script has and exactly what the running app
never has — its main window is always open, so the app's export path was not affected. Run the
script at a gate and after an Electron upgrade: with the pre-fix `destroy()` put back it fails at
render 2, which is how it was verified to bite, and the app spec's export test passes either way.
