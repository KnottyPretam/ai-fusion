...
saveDomSnapshot(slot): Promise<{path}>                                                  // invoke 'panes:snapshot' (scrubbed HTML under userData/snapshots/<slot>-<ts>.html). The writer (ipc.js `saveDomSnapshot({client, name, snapshotsDir})`) is view-agnostic since S11: main calls it itself with the hidden analyst's client, as `analyst-<ts>.html`, BEFORE a settled re-read of a failed analyst capture (the newest 20 `analyst-*.html` are kept); `requireSlot` stays at this IPC boundary, so the renderer can only ever name a pane
// Stage 3:
...
```
The prompt is always a message field, never interpolated into code; the adapter never clears the composer on failure; one op in flight per view (second → `busy`). A `cancel` that main's OWN timeout sends is tracked (S11, adapter-client.js): the preload frees its in-flight slot only once it has answered it, so that answer is awaitable as `AdapterRequestError.cancelResult` and the orchestrator waits for it before asking that view anything else — a `snapshot` or a `ready` sent earlier would be `busy`. Settled re-read (S11, orchestrator.js): an analyst turn on a structured purpose (`extraction` / `defense` / `convergence`) whose observe fails `timeout` or `reply_not_found` saves the failing DOM (above), reloads the chat the view is in (only a URL matching the site's `chatUrlPattern` — never the site root, never chatgpt's `/c/WEB:<uuid>` placeholder), asks `ready`, then `observe{baselineCount: <the pre-submit assistantCount>, expect:'json', quietMs, settleMs, firstTokenMs, timeoutMs: 60000}` and, when that reads the answer, returns the ordinary `result ok:true captured:true` frame inside the SAME request; any failure in the recovery returns the original failure frame, `partial` and all. Never for a pane, never for `chat`. `adapter:config.selectors` is the full merged config object (`{version, chatgpt, claude, grok}`); `zoom(slot, direction)` takes two arguments and every single-slot method takes the bare slot string.
...
createAdapter({document, window, site, selectors, now = Date.now}) → {
  ...
  observe({baselineCount, quietMs, timeoutMs, firstTokenMs?, settleMs?, expect?, incompleteGraceMs?, signal}) → Promise<{text, doneBy, ms}>,   // Stage 2; also snapshots the containers present when it starts and prefers the last one NOT in that set, so a re-render cannot hand back the previous turn's text; an end signal is withdrawn by a visible stop button and honoured only after the settle window, and a `done` match inside the reply body never counts. Every failing observe message ends in ONE bracketed diag line (S11) — `[containers=N followed=<i|-1> connected=<bool> stopNow=<bool> stopSeen=<bool> end=<signal|-> md=N pre=N code=N reply=N inner=N textContent=N maxContainer=N rect=<w×h|-> viewport=<w×h|->]`, counts and booleans only, never reply text — so `inner ≫ reply` points at the reader (the `assistantText` cascade), `maxContainer ≫ inner` at the container choice, and all small at the DOM; the code and the `partial` are what they always were
}
