...
| _issue 3, desktop half_ | 2026-09-22 | Recover a lost analyst capture from the SETTLED page, and make the failure diagnose itself. Three read-only probes (2026-09-20/21/22) found the complete answer in the analyst chat after every failed capture, so the plan's C1 (an in-adapter re-sample of the same live DOM at S10) is joined by a **settled re-read** in the orchestrator's failure funnel: for an analyst turn on a structured purpose that fails `timeout` / `reply_not_found`, await the adapter's answer to main's own cancel (`AdapterRequestError.cancelResult`, tracked in adapter-client.js — the preload holds one op in flight and answered `busy` to anything sent sooner), save the failing DOM scrubbed (`saveFailureSnapshot` → `snapshots/analyst-<ts>.html`, newest 20 kept; `saveDomSnapshot` is view-agnostic with `requireSlot` kept at the IPC boundary), reload the chat (a `chatUrlPattern` match only), `ready`, `observe` from the PRE-SUBMIT baseline with `expect:'json'` under `SETTLED_REREAD_MS` = 60 s, and return an ordinary captured result inside the same request; any failure there returns the original frame with its partial. Added beyond the spec: the re-read is skipped when what is left of the backend's grant cannot hold it (the view would still be reserved when the correction attempt arrived → `view_busy`) and when the submit carried no baseline (a reloaded page would count the settled answer as an earlier turn). **Diagnosis:** every failing observe message ends in one `diag()` line of counts and booleans (never reply text): `inner ≫ reply` → the reader, `maxContainer ≫ inner` → the container choice, all small → the DOM. **Grace:** `incompleteGraceMs` is an observe message field, sent 20 s × `ANALYST_PATIENCE` for the analyst view only. One log line per submit names `confirmedBy` and the send selector. A stale `adapters` assertion (`observe.spec.js:1059`, written before the S10 message rewording at `1fc9339`) was corrected in passing. |
| _issue 3, desktop review_ | 2026-09-22 | The adversarial review of the desktop half, applied. **Reserve under the ceiling:** `budgets()` now subtracts the whole re-read window (`rereadReserveMs(readyMs)` = navigate wait + ready + two graces + `SETTLED_REREAD_MS`, floored at half the ceiling) from an analyst capture, so the recovery can never be starved by the capture that preceded it; the skip check uses the same sum. **Still writing:** a `timeout` whose diag line says `stopNow=true` is NOT re-read — the site's stop control was still up at the last sample, the model is mid-answer, and a reload would abandon it (the snapshot is still taken). **Cancel ack order:** the preload answers a matched `cancel` only after the aborted op has itself answered and freed the in-flight slot (`inFlight.settled`), so an awaited cancel is a guarantee that the next op cannot meet `busy`; the loop's own `loadUrl` is raced against `NAVIGATION_WAIT_MS` and `settledReread` can never reject into the funnel. **Pane snapshots:** `saveFailureSnapshot` only ever targets the analyst view (a pane never recovers, so its dead branch went) and `pruneSnapshots` ignores pane names. **Diag:** `followed=-` when no container was ever picked, `-1` only for a followed node that is gone; the line is appended only to the `timeout` / `reply_not_found` family — a session refusal mid-capture stays the bare phrase the session rules promise. Verification: desktop unit 533/533; `adapters` 133/133 run ALONE (the three earlier failures were the app spec running beside it); `app` spec 20/20 on the free port set (5198/5185/8022) beside the installed build; `tests/live` 9/9 in 50 s on the funded key — the first live run since `complete_json` gained `on_partial` (`87e011f`). |
| _preparse_ | 2026-09-23 | **Pre-parse**, a preview step before Send. Trigger: the live run on `a18c67a9` (Refactor `ok` in 7 calls / 11 min; the refactored replies carried 51/82/87 claims, so Analyze still condensed in 7 calls before its comparison, 14 min) — every analyst call is a real reply and reply LENGTH sets the call count. Decided with the user: a **preview button** (the composer text is rewritten in place, reviewed, then sent — Send stays byte-verbatim), the **analyst chat** as the rewriter (one `validated_call`, the primitive promoted from `refactor._call`), a **4,000-character** reply cap in a deterministic `ANSWER_FORMAT` block (at most 8 claims with one-sentence reasons, `Uncertain:`) appended LAST. Three design reviews settled: a per-conversation route (`analystContinuation(seam, '')` would open a new chat for the correction attempt), `createAdopted()` so a first Pre-parse creates the conversation WITHOUT navigating the panes (a plain create trips `useOpenChats`), and an INLINE cancellable call — the one exception to the producer-task rule, because nothing is persisted and Starlette's disconnect cancellation plus the bridge's `aclosing` turn Cancel into a real `cancel` frame (pinned by an ASGI-level disconnect test; depends on ASGI spec 2.3, CLAUDE.md gotcha 21). Found during implementation: the streaming task no longer holds the router's `conversation_scope`, so the generator re-enters it (otherwise the frame carried `conversation_id: null`); the first-Send title must come from `strip_format(prompt)` (a short restated question would title as the block's first line — caught by review, fixed in `send.py`). Part B: Refactor asks for at most 12 claims (per piece when chunked) and keeps any overshoot with one WARNING. Verification: backend 1793 (+41), frontend 616 (+31), desktop unit 533, `adapters` 133/133 alone (the canned-key table gained the pre-parse row), `app` spec 21/21 on the free port set (the new test drives button → analyst view → composer → Send); goldens unmoved. |
