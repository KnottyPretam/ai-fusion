// menu.js — the Site menu (Reload selectors, Save DOM snapshot, Sign out of <site>) is added to the
// shortcuts template before Edit; every item drives its action; failures are logged, never thrown.
...
function setup({ dev = true, active = 'grok', failSnapshot = false } = {}) {
  const calls = []
  const log = fakeLog()
...
test('the template keeps Triplex / Panes / Edit and inserts Site before Edit with the five items', () => {
  ...
  assert.deepEqual(
    ...
    ['Reload selectors', 'Save DOM snapshot of the active pane', 'separator', 'Sign out of Claude', 'Sign out of ChatGPT', 'Sign out of Grok'],
  )
...
test('clicking the items drives the actions: reload selectors, snapshot of the ACTIVE pane, sign out of each site', async () => {
  ...
  await tick()
  await tick()
  assert.deepEqual(calls, [['reloadSelectors'], ['saveSnapshot', 'claude'], ['signOut', 'grok'], ['signOut', 'chatgpt'], ['reload', 'claude']])
