...
describe('SendPane', () => {
  ...
  test('solo continue posts to the slot endpoint under feature send and refetches', async () => {
    const existing = conv({
      threads: { claude: [msg('user', 'q0'), msg('assistant', 'a0')], chatgpt: [msg('user', 'q0'), msg('assistant', 'a0')], grok: [msg('user', 'q0'), msg('assistant', 'a0')] },
      turns: [{ id: 't0', type: 'send', prompt: 'q0', slot_config: CFG, responses: { claude: 'a0', chatgpt: 'a0', grok: 'a0' }, usage: { calls: [], totals: {} } }],
    })
    ...
    const calls = stubFetch([
      ...
      {
        method: 'POST',
        url: '/api/conversations/c1/slots/grok/continue',
        respond: () => sse([{ type: 'turn_start', turn_id: 't1', feature: 'continue', slots: ['grok'] }, sample.slotStart('grok'), sample.slotDelta('grok', 'grok goes deeper'), sample.slotDone('grok'), sample.turnDone('t1')]),
      },
      { method: 'GET', url: '/api/conversations/c1', respond: json(after) },
    ])
    renderWithStore(<SendPane />, { preloaded: { conversation: existing, slotConfig: CFG } })
    ...
    expect(calls[calls.length - 2].body).toEqual({ prompt: 'deeper' })
    expect(await screen.findByText('grok goes deeper')).toBeInTheDocument()
    expect(screen.getAllByTestId('slot-grok-message')).toHaveLength(4)
