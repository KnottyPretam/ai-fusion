...
const PROMPT = 'What is the maximum gyroscope full-scale range of the Bosch BMI088 IMU?'
const SHOT = (name) => ({ path: `../docs/screenshots/${name}.png`, fullPage: true })
...
test('send, analyze, fuse, continue', async ({ page }) => {
  ...
  await expect(page.getByTestId('analyze-report')).not.toContainText(/claude|chatgpt|grok|openai|anthropic/i)
  ...
  await expect(page.getByTestId('fusion-timeline')).not.toContainText(/claude|chatgpt|grok|openai|anthropic/i)
